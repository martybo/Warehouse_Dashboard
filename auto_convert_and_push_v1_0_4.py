...
# Ensure all NaN values are converted to empty strings BEFORE parsing to JSON
...
try:
    ...
    repo.update_file(
        existing_file.path,
        "v1.0.4: Fix NaN JSON issue + defensive columns",
        contents,
        existing_file.sha
    )
    ...
    repo.create_file(
        target_path_in_repo,
        "Initial stock JSON upload (v1.0.4)",
        contents
    )
